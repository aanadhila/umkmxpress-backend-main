...
@section('content')
    <div class="card card-docs flex-row-fluid mb-2">
        ...
        <div class="card-body pt-0">
            <table id="senders-table" class="table align-middle table-row-dashed fs-6 gy-5">
                <thead>
...
@push('js')
    <script>
        var datatable = $('#senders-table').DataTable({
            processing: true,
            serverSide: true,
            ordering: true,
            stateSave: false,
        ...
        $(document).on("click", "#deleteConfirm", function(e) {
            ...
            var text = 'Apakah anda yakin ingin menghapus sender ":name"?'
            Swal.fire({
                customClass: {
                ...
                if (result.isConfirmed) {
                    e.preventDefault();
                    var id = $(this).data("id");
                    var route = "{{ route('senders.destroy', ':id') }}";
                    $.ajax({
    ...
    @include('components.gmaps', ['function' => 'initSenderMap'])
@endpush
