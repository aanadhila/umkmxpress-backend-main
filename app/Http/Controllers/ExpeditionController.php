...
class ExpeditionController extends Controller
{
    public function index()
    {
        if (request()->ajax()) {
            ...
            return DataTables::of($expeditions)
                ...
                ->editColumn('status', function ($item) {
                    ...
                    return '<div class="form-check form-check-solid form-switch form-check-custom fv-row">
                                <input class="form-check-input w-45px h-30px" type="checkbox" id="changeStatus" data-id="' . $item->id . '" ' . $checked . '>
                                <label class="form-check-label" for="status"></label>
                            </div>';
                })
                ->editColumn('price', function ($item) {
                    return $item->price ? 'Rp ' . number_format(intval($item->price), 0, ',', '.') : '-';
                })
                ->editColumn('price_km', function ($item) {
                    return $item->price_km ? 'Rp ' . number_format(intval($item->price_km), 0, ',', '.') : '-';
                })
                    ...
                    return '<div class="dropdown">
                                ...
                                <div class="dropdown-menu menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-50px py-4" aria-labelledby="menu-' . $item->id . '">
                                    <div class="menu-item px-3">
                                        <a class="menu-link px-3" id="editExpedition" data-bs-toggle="modal" data-bs-target="#editExpeditionModal" data-id="' . $item->id . '">Edit</a>
                                    </div>
                                    <div class="menu-item px-3">
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $price      = $request->price_type == 'price' ? $request->price : null;
            $price_km   = $request->price_type == 'price_km' ? $request->price : null;
            Expedition::create([
                'name'          => $request->name,
                'icon'          => $request->file('icon') ? $icon_path : null,
            ...
            DB::commit();
            return response()->json(['message' => 'Ekspedisi berhasil disimpan!'], 200);
        } catch (\Throwable $th) {
            DB::rollBack();
    ...
    public function edit($id)
    {
        try {
            ...
            $expedition->icon = $expedition->pict;
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $expedition->update([
                'name'          => $request->name ?? $expedition->name,
                'icon'          => $request->file('icon') ? $icon_path : $expedition->icon,
                'description'   => $request->description ?? $expedition->description,
                'started_at'    => $request->started_at ?? $expedition->started_at,
                'ended_at'      => $request->ended_at ?? $expedition->ended_at,
                'price'         => $request->price_type == 'price_km' ? null : $request->price,
                'price_km'      => $request->price_type == 'price' ? null : $request->price,
                'status'        => $request->status ? 1 : 0,
            ]);
            DB::commit();
            return response()->json(['message' => 'Ekspedisi berhasil disimpan!'], 200);
    ...
    public function destroy($id)
    {
        ...
        try {
            $expedition = Expedition::findOrFail($id);
            if ($expedition->shipments()->count() > 0) return response()->json(['message' => 'Data ekspedisi ini memiliki data pemesanan!'], 500);
    ...
    public function changeStatus(Request $request)
    {
        DB::beginTransaction();
        try {
            $expedition = Expedition::findOrFail($request->id);
            $expedition->update([
                'status'    => $request->status,
            ]);
            DB::commit();
            return response()->json(['message' => 'Ekspedisi berhasil diupdate'], 200);
        } catch (\Throwable $th) {
            DB::rollBack();
            return response()->json(['message' => $th->getMessage()], 500);
        }
